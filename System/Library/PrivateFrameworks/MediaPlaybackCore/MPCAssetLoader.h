//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MPCPlayerItemConfigurator, NSOperationQueue;
@protocol MPCAssetLoaderDelegate;

__attribute__((visibility("hidden")))
@interface MPCAssetLoader : NSObject
{
    id <MPCAssetLoaderDelegate> _delegate;	// 8 = 0x8
    MPCPlayerItemConfigurator *_configurator;	// 16 = 0x10
    NSOperationQueue *_loadingQueue;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x00000000001ad328
@property(readonly, nonatomic) NSOperationQueue *loadingQueue; // @synthesize loadingQueue=_loadingQueue;
@property(retain, nonatomic) MPCPlayerItemConfigurator *configurator; // @synthesize configurator=_configurator;
@property(nonatomic) __weak id <MPCAssetLoaderDelegate> delegate; // @synthesize delegate=_delegate;
- (double)unloadingPlaybackAssetsOnIdleDuration;	// IMP=0x00000000001ad285
- (void)cancelLoading;	// IMP=0x00000000001ad248
- (void)loadAssetValuesFor:(id)arg1 keys:(id)arg2 completion:(CDUnknownBlockType)arg3;	// IMP=0x00000000001ad0d8
- (void)loadAssetFor:(id)arg1 token:(long long)arg2 completion:(CDUnknownBlockType)arg3;	// IMP=0x00000000001ace2e
- (id)initWithDelegate:(id)arg1;	// IMP=0x00000000001acd6a

@end

