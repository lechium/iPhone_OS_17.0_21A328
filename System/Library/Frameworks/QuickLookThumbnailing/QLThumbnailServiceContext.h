//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <Foundation/NSExtensionContext.h>

@class NSString, QLThumbnailProvider;

__attribute__((visibility("hidden")))
@interface QLThumbnailServiceContext : NSExtensionContext
{
    QLThumbnailProvider *_thumbnailProvider;	// 8 = 0x8
}

+ (id)_extensionAuxiliaryHostProtocol;	// IMP=0x0010000000020840
+ (id)_extensionAuxiliaryVendorProtocol;	// IMP=0x0010000000020678
- (void).cxx_destruct;	// IMP=0x00000000000217b4
@property(retain, nonatomic) QLThumbnailProvider *thumbnailProvider; // @synthesize thumbnailProvider=_thumbnailProvider;
- (id)_thumbnailGenerationQueue;	// IMP=0x0000000000021733
- (void)generateThumbnailOfSize:(struct CGSize)arg1 minimumSize:(double)arg2 scale:(double)arg3 badgeType:(unsigned long long)arg4 iconFlavor:(int)arg5 contentType:(id)arg6 interpolationQuality:(int)arg7 wantsLowQuality:(_Bool)arg8 withFileURLHandler:(id)arg9 additionalResourcesWrapper:(id)arg10 generatorData:(id)arg11 completionHandler:(CDUnknownBlockType)arg12;	// IMP=0x0000000000021260
- (void)provideThumbnailForRequest:(id)arg1 completionHandler:(CDUnknownBlockType)arg2;	// IMP=0x0000000000020f54
- (void)generateThumbnailOfSize:(struct CGSize)arg1 minimumSize:(double)arg2 scale:(double)arg3 badgeType:(unsigned long long)arg4 iconFlavor:(int)arg5 contentType:(id)arg6 interpolationQuality:(int)arg7 wantsLowQuality:(_Bool)arg8 withContents:(id)arg9 additionalResourcesWrapper:(id)arg10 generatorData:(id)arg11 completionHandler:(CDUnknownBlockType)arg12;	// IMP=0x0000000000020dbf
- (void)ioSurfaceForRequest:(id)arg1 withSize:(struct CGSize)arg2 completion:(CDUnknownBlockType)arg3;	// IMP=0x0000000000020abf
- (id)protocolHostWithErrorHandler:(CDUnknownBlockType)arg1;	// IMP=0x0000000000020965
- (id)protocolHost;	// IMP=0x00000000000208ac

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

